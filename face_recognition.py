...
# x - values are stored in numpy arrays0
...
def knn(train, test, k=5):
    ...
    dk = sorted(dist, key=lambda data: data[0])[:k]
...
dataset_path = r"/Users/rahesh/Desktop/DataScience/DataScienceProjectsWithAzure/Git_codes/Face-Detection-And-Recognition/data/"
labels = []
class_id = 0
...
traindata = np.concatenate((X, Y.reshape(-1, 1)), axis=1)
...
while True:
    ...
    key_pressed = cv2.waitKey(1) & 0xFF  # A bit masking to get last 8 bits
    ...
    for face in faces:
        ...
        pred = knn(traindata, face_section.flatten())
        name = names[int(pred)]
